function logpdf = LRM_IPPosterior(bs2,y, x, b_prior, V_prior, aa_prior, bb_prior)
	...
	logprobbeta  = log(mvnpdf(b,b_prior,V_prior));
    % Prior probability for sigma. Notes (5.1)
    logprobsigma = -(aa_prior/2+1)*log(s2) - (bb_prior/2)/s2;
